...
* **FR-GH.1** **Authentication.** v1 uses a **classic personal access token** with the `repo` scope, entered once and stored in `localStorage` under the same key the preview tool uses (single setup step covers both tools). The UI MUST link to the token-creation instructions in the Preview Setup guide. Classic rather than fine-grained is deliberate as of 2026-07-30: a fine-grained token is scoped to one resource owner and cannot reach a repo the author only collaborates on, so commits fail with 403 while reads may succeed — which contradicts the editor's premise of working across any repo the author can push to. OAuth device flow is a Future Direction (needs no secret but requires a CORS proxy for the token endpoint).
